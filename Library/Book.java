...
//curl -X POST http://localhost:8080 -H "Content-Type: application/json" -d '{"prompt": "# password generator\n\n", "language": "python"}'
// import http.server
// import threading
// import requests
// import json
...
// def build_code_snippet(prompt, language='python'):
//     """Build a complete code snippet by iteratively querying the API."""
//     current_prompt = prompt
//     result = ''

//     completion = copilot(current_prompt, language)
...
//         completion = copilot(current_prompt+'\n', language)
        
//     return result+'\n'

// # Check if the token is invalid through the exp field
// def is_token_invalid(token):
//     if token is None or 'exp' not in token or extract_exp_value(token) <= time.time(): # type: ignore
//         return True
//     return False
...
//         completion = build_code_snippet(prompt, language)
